...
def genetic_algorithm():
    ...
    total_weight = sum(weights[i] for i in range(len(best_individual)) if best_individual[i])
    return fitness(best_individual), total_weight, [names[i] for i in range(len(best_individual)) if best_individual[i]]

max_value, total_weight, items_chosen = genetic_algorithm()

print(f"Valor máximo possível: {max_value}")
print(f"Peso total carregado: {total_weight} kg")
